...
variable "image" { type=string default="ai-observability-platform:latest" }
resource "kubernetes_deployment" "api" {
  metadata { name="ai-observability-platform" namespace=var.namespace labels={app="ai-observability-platform"} }
  spec { replicas=var.replicas selector { match_labels={app="ai-observability-platform"} } template {
    metadata { labels={app="ai-observability-platform"} }
...
resource "kubernetes_service" "api" {
 metadata {name="ai-observability-platform" namespace=var.namespace}
 spec { selector={app="ai-observability-platform"} port {port=80 target_port=8000} }
}
